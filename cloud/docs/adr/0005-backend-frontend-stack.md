# 0005 — Cloud backend is NestJS + Postgres + OpenAuth.js on SST/AWS; frontend is Next.js + Chart.js

We looked at opencode's own hosted console (same problem: accounts, billing, auth, usage dashboards), which uses SST v3 + Cloudflare Workers + Postgres/Drizzle + OpenAuth.js on the backend, and SolidJS/solid-start + Vite + Chart.js on the frontend. Framework choice: NestJS instead of Workers-shaped functions (matches the team's existing NestJS/Next.js/axios conventions), Next.js instead of SolidJS (larger hiring pool, same axios-generic-interceptor API client pattern used elsewhere). OpenAuth.js and Chart.js carry over unchanged — both framework-agnostic, both already proven for this exact problem.
