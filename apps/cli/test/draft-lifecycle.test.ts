...
function makeHarness(opts?: { answer?: string }): Harness {
  ...
  const readTool = createReadTool({
    draftManager,
    readDocument: async (file) => JSON.parse(await execCli(['get', file])).data.content,
  });
...
describe('draft lifecycle end-to-end', () => {
  ...
  test('revert replants a snapshot as a draft; accept writes it', async () => {
    ...
    const get = await tool.execute({ command: 'get', file: realFile }, { sessionID: 'sess-a' });
  ...
  test("cross-session orphan recovery: accept via prompt writes the orphan's edits", async () => {
    ...
    const get = await b.tool.execute({ command: 'get', file: realFile }, { sessionID: 'sess-b' });
    expect(get.success).toBe(true);
  ...
  test('read tool sees draft content once a draft exists', async () => {
    ...
    const result = await readTool.execute({ file: realFile }, { sessionID: 'sess-a' });
  ...
  test('read tool reads a new-file draft before the real file exists', async () => {
    ...
    await tool.execute({ command: 'create', file: NEW }, { sessionID: 'sess-a' });
    const result = await readTool.execute({ file: NEW }, { sessionID: 'sess-a' });
    expect(result.success).toBe(true);
    if (!result.success) {
