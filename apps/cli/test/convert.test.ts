...
describe('convert tool', () => {
  test('asks user before converting', async () => {
    ...
    const result = await tool.execute({ file: 'report.doc' }, { sessionID: 'test' });
  ...
  test('honors explicit format override', async () => {
    ...
    await tool.execute({ file: 'report.doc', format: 'pptx' }, { sessionID: 'test' });
    expect(convertedFormat).toBe('pptx');
  });

  test('rejects non-legacy source files', async () => {
    const tool = makeTool();
    const result = await tool.execute({ file: 'report.docx' }, { sessionID: 'test' });
    expect(result.success).toBe(false);
    if (!result.success) {
  ...
  test('rejects unknown legacy extensions', async () => {
    const tool = makeTool();
    const result = await tool.execute({ file: 'archive.rtf' }, { sessionID: 'test' });
    expect(result.success).toBe(false);
    if (!result.success) {
  ...
  test('integration: converts real .doc via soffice when available', async () => {
    const have = (cmd: string) =>
      Bun.spawnSync(['which', cmd], { stdout: 'pipe', stderr: 'pipe' }).exitCode === 0;
    ...
    execFileSync(
      'soffice',
      ['--headless', profileFlag('lo-profile-down'), '--convert-to', 'doc', '--outdir', dir, docx],
      {
        encoding: 'utf-8',
        timeout: 60000,
      },
