...
describe('normalizeMcpResult', () => {
  ...
  test('falls back to JSON for empty content', () => {
    expect(normalizeMcpResult({ content: [] })).toBe(JSON.stringify({ content: [] }));
  });
...
describe('normalizeMcpContents', () => {
  ...
  test('throws on binary (blob) content', () => {
    expect(() => normalizeMcpContents([{ uri: 'mem://a', blob: 'AAAA' }])).toThrow(
      'binary content',
    );
  });
...
`,
  ...
  test('lists prompts and resources and reads a resource', async () => {
    ...
    expect(await client!.readResource('mem://notes/hello')).toBe('hello resource');
  });
  ...
  test('cursor-paginated servers return every item', async () => {
    ...
    try {
      expect((await paginated.listTools()).map((t) => t.name)).toEqual(['t1', 't2', 't3']);
      expect((await paginated.listPrompts()).map((p) => p.name)).toEqual(['p1', 'p2', 'p3']);
