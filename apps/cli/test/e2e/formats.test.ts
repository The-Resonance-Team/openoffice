...
describe("officecli integration on all three formats", () => {
  test.skipIf(skip)(
    "docx: create → add paragraph → accept → read back",
    async () => {
  ...
  test.skipIf(skip)(
    "xlsx: create → set cell → accept → read back",
    async () => {
  ...
  test.skipIf(skip)(
    "pptx: create → add slide + shape → accept → read back",
    async () => {
      ...
      await draftManager.accept(SESSION, file);
      expect(
        JSON.stringify(runOfficecli(["get", file, "/slide[1]"]))
      ).toContain("Gamma");
    }
  );
});
