...
describe('config loading', () => {
  ...
  test('invalid schema throws with the file path', () => {
    const dir = tempDir();
    const path = join(dir, 'openoffice.json');
    writeFileSync(path, JSON.stringify({ provider: { openai: { apiKey: 123 } } }));
    expect(() =>
      resolveConfig({
        globalPath: join(dir, 'missing.json'),
        projectPath: path,
  ...
  test('project layer overrides global, nested provider records merge', () => {
    ...
    writeFileSync(project, JSON.stringify({ model: 'new', provider: { openai: { apiKey: 'p' } } }));
    const config = resolveConfig({
      globalPath: global,
      projectPath: project,
      env: emptyEnv,
  ...
  test('env: references resolve from the environment', () => {
    const dir = tempDir();
    const path = join(dir, 'openoffice.json');
    writeFileSync(path, JSON.stringify({ provider: { openai: { apiKey: 'env:OPENAI_KEY' } } }));
    const config = resolveConfig({
      globalPath: join(dir, 'missing.json'),
      projectPath: path,
  ...
  test('missing env: anywhere else is still a hard error', () => {
    const dir = tempDir();
    const path = join(dir, 'openoffice.json');
    writeFileSync(path, JSON.stringify({ office: { managedDocumentsFolder: 'env:OOO_DOCS' } }));
    expect(() =>
      resolveConfig({
        globalPath: join(dir, 'missing.json'),
        projectPath: path,
  ...
  test('~/ expands in string values', () => {
    const dir = tempDir();
    const path = join(dir, 'openoffice.json');
    writeFileSync(path, JSON.stringify({ office: { managedDocumentsFolder: '~/docs' } }));
    const config = resolveConfig({
      globalPath: join(dir, 'missing.json'),
      projectPath: path,
      env: emptyEnv,
...
describe('JSONC stripping', () => {
  test('strips comments and trailing commas, preserves strings', () => {
    ...
    expect(
      (parsed as unknown as { provider: { openai: { apiKey: string } } }).provider.openai.apiKey,
