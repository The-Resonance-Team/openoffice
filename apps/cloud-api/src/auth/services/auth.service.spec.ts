...
describe("AuthService", () => {
  ...
  const register = (
    ...
    service.register(
      { email, password, name: "A", orgName } as RegisterDto,
      "1.2.3.4"
    );

  const login = (dto: Partial<LoginDto> = {}) =>
    service.login(
      { email: "a@x.dev", password: "password123", ...dto } as LoginDto,
      "1.2.3.4"
    );
  ...
  describe("register", () => {
    ...
    it("stores the email lowercased and rejects a duplicate", async () => {
      ...
      await expect(register("a@x.dev", "Other")).rejects.toThrow(
        /already registered/
      );
  ...
  describe("refresh", () => {
    ...
    it("revokes the session when a pre-rotation token is replayed (reuse detection)", async () => {
      ...
      await expect(
        service.refresh(first.refreshToken, "1.2.3.4")
    ...
    it("rejects unknown and expired sessions", async () => {
      ...
      await expect(service.refresh("deadbeef", "1.2.3.4")).rejects.toThrow();

      const session = [...db._session.values()].find(
        (s: any) => s.hashedRefresh === sha256(refreshToken)
      );
      session.expiresAt = new Date(Date.now() - 1_000);
      await expect(service.refresh(refreshToken, "1.2.3.4")).rejects.toThrow();
    });
  });
