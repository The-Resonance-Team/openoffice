...
describe('Cloud API (e2e)', () => {
  ...
  const cookiesOf = (res: any): string[] => res.headers['set-cookie'] as string[]
  ...
  beforeAll(async () => {
    ...
    await request(app.getHttpServer()).post('/v1/auth/verify-email').send({ token }).expect(201)
    const res: any = await request(app.getHttpServer())
      .post('/v1/auth/login')
  ...
  it('GET /v1/health is public; other routes require a JWT or API key', async () => {
    const health: any = await request(app.getHttpServer()).get('/v1/health').expect(200)
  ...
  it('login blocks unverified accounts until verify-email is consumed', async () => {
    ...
    const token = await app.get(EmailTokenService).createToken(dup!.id, EmailTokenType.VERIFY_EMAIL)
    await request(app.getHttpServer()).post('/v1/auth/verify-email').send({ token }).expect(201)
    const res: any = await request(app.getHttpServer())
      .post('/v1/auth/login')
      .send({ email: email('dup'), password })
  ...
  it('Daemon API keys: create once, list, authenticate via x-api-key, revoke', async () => {
    ...
    await request(app.getHttpServer())
      ...
      .set('Cookie', sharedCookies)
      .expect(200)
    await request(app.getHttpServer()).get('/v1/auth/me').set('x-api-key', rawKey).expect(401)
  })
  ...
  it('GET /v1 accepts a JWT from the `token` cookie', async () => {
    ...
    await request(app.getHttpServer()).get('/v1').set('Cookie', `token=${token}`).expect(200)
  })
})
