...
const server = Bun.serve({
  ...
  async fetch(req) {
    ...
    if (url.pathname === '/api/sessions' && req.method === 'POST') return json(200, { id: 's1' });
    if (url.pathname === '/api/sessions/missing') return json(404, { error: 'not found' });
    if (url.pathname === '/api/update') return json(200, { check: true, available: false });
    if (url.pathname.endsWith('/share'))
      return ++shareCalls === 1 ? json(200, { url: 'https://x' }) : json(500, { error: 'nope' });
    if (url.pathname.endsWith('/unshare'))
      return ++unshareCalls === 1 ? json(200, {}) : json(500, { error: 'nope' });
...
test('stream dispatches every event type and stops on abort', async () => {
  ...
  stop();
  expect(seen).toEqual(['token', 'toolStart', 'toolDone', 'message', 'ask', 'done']);
});
