...
describe('document search extras', () => {
  ...
  test('reads Excel metadata and structure in one ExcelJS pass', async () => {
    ...
    expect(result.structured).toContain('Excel formula: Sheet1!A1 = SUM(B1:B2)');
    expect(result.structured).toContain('Excel comment: Sheet1!A1 = review budget');
  });

  test('uses metadata fallback for legacy Office files', async () => {
    for (const extension of ['.doc', '.xls', '.ppt']) {
      let calls = 0;
      const result = await readDocumentSearchData(`legacy${extension}`, async () => {
        calls++;
        return { Author: 'Alice' };
      });
  ...
  test('falls back to ExifTool metadata when PDF.js fails', async () => {
    const result = await readDocumentSearchData(join(tempDir(), 'broken.pdf'), async () => ({
      Author: 'Alice',
    }));

    expect(result.metadata?.Author).toBe('Alice');
    expect(result.notes).toContain('PDF.js search failed; used ExifTool metadata fallback');
  });

  test('falls back to ExifTool metadata when ExcelJS fails', async () => {
    const result = await readDocumentSearchData(join(tempDir(), 'broken.xlsx'), async () => ({
      Author: 'Alice',
    }));

    expect(result.metadata?.Author).toBe('Alice');
    expect(result.notes).toContain('ExcelJS search failed; used ExifTool metadata fallback');
  });
});
