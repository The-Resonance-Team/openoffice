...
import { resolveCredential, resolveModel, AuthRequiredError } from '../providers';
...
describe('resolveCredential — resolution order', () => {
  ...
  test('declared provider with no key anywhere raises a clear error naming both fixes', () => {
    expect(() =>
      resolveCredential(anthropicConfig(undefined).provider?.anthropic, 'anthropic', store),
  ...
  test('undeclared provider with no stored credential and no env var throws AuthRequiredError', () => {
    ...
    try {
      expect(() => resolveCredential(undefined, 'anthropic', store)).toThrow(AuthRequiredError);
    } finally {
...
describe('resolveModel', () => {
  test('declared provider with nothing resolving raises the clear error', () => {
    expect(() =>
      resolveModel('anthropic/claude-sonnet-4-20250514', anthropicConfig(undefined), store),
  ...
  test('unknown provider is rejected before any credential lookup', () => {
    expect(() => resolveModel('nope/gpt-0', anthropicConfig('sk-config'), store)).toThrow(
      /Unknown provider "nope"/,
    );
  });

  test('malformed model string is rejected', () => {
    expect(() => resolveModel('no-slash', anthropicConfig('sk-config'), store)).toThrow(
      /provider\/model-id/,
    );
  });
});
