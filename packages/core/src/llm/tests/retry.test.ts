...
describe('classifyRetryable', () => {
  ...
  test('429 marked retryable by the SDK is retried', () => {
    expect(classifyRetryable(apiError(429, { isRetryable: true })).retry).toBe(true);
  });
  ...
  test('rate-limit text patterns are retried', () => {
    expect(classifyRetryable(new Error('Rate limit exceeded')).retry).toBe(true);
  ...
  test('JSON error bodies are parsed for rate-limit codes', () => {
    ...
    expect(
      classifyRetryable(new Error(JSON.stringify({ code: 'rate_limit_exceeded' }))).retry,
    ).toBe(true);
    expect(classifyRetryable(new Error(JSON.stringify({ code: 'insufficient_quota' }))).retry).toBe(
      false,
    );
  });
