...
describe('DraftManager', () => {
  test('first mutating call creates a draft copy, meta, and lock; real untouched', async () => {
    ...
    expect(existsSync(join(dir, 'locks', `${filePathHash(REAL)}.json`))).toBe(true);
    const meta = JSON.parse(readFileSync(`${draftPath(SESS_A)}.meta.json`, 'utf-8'));
  ...
  test('read verbs return the draft once one exists, else the real path', async () => {
    ...
    await mgr.resolve(REAL, SESS_A, true);
    expect((await mgr.resolve(REAL, SESS_A, false)).path).toBe(draftPath(SESS_A));
  });
  ...
  test('open and create verbs create the draft like mutating verbs', async () => {
    ...
    expect(existsSync(join(dir, 'drafts', filePathHash(NEW_FILE), 'sess-a.docx'))).toBe(true);
  });
  ...
  test("stale-lock override: B takes over, A's next mutation errors, A's draft marked orphaned", async () => {
    ...
    await mgr.resolve(REAL, SESS_A, true);
    now += 25 * 60 * 60 * 1000;
    expect((await mgr.resolve(REAL, SESS_B, true)).path).toBe(draftPath(SESS_B));
    const aMeta = JSON.parse(readFileSync(`${draftPath(SESS_A)}.meta.json`, 'utf-8'));
