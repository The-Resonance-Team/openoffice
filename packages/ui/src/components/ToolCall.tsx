...
function toolInfo(tool: string, params: unknown): { label: string; icon: IconName } {
  const p = asParams(params);
  const file = typeof p.file === 'string' ? basename(p.file) : undefined;
  switch (tool) {
    case 'officecli':
    ...
    case 'glob':
      return {
        label: typeof p.pattern === 'string' ? `Searching for ${p.pattern}` : 'Searching files',
        icon: 'search',
      };
    case 'grep':
      return {
        label:
          typeof p.query === 'string' ? `Searching for "${p.query}"` : 'Searching file contents',
        icon: 'search',
      };
    case 'question':
...
export function ToolCallCard({ part }: { part: Extract<ChatPart, { kind: 'tool' }> }) {
