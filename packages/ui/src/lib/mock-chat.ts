...
export function pickMockReply(userText: string): string {
  ...
  if (lower.includes('revenue') || lower.includes('bridge')) return replyPool[0];
  if (lower.includes('summary') || lower.includes('variance')) return replyPool[1];
  if (lower.includes('deck') || lower.includes('slide') || lower.includes('risk'))
    return replyPool[2];
  if (lower.includes('appendix') || lower.includes('table')) return replyPool[3];
